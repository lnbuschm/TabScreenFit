...
namespace TabScreenFit
{
    class KMeans
    {
        public static int MAX_CLUSTER_SEARCH = 6;

        public static void Main_method(double[][] args)
        {
            Console.WriteLine("\nBegin k-means clustering demo\n");
            double[][] rawData = args;

            // realdata likely to come from a text file or SQL
            /*
            ...
            */
            ...
            BetweenGroupVariance(rawData);
            // fo


        } // Main

        private static double[] BetweenGroupVariance(double[][] rawData)
        {
            double[] groupVariation = new double[MAX_CLUSTER_SEARCH];

            //    ShowClustered(rawData, clustering, numClusters, 1);
            double grandMean = GetGrandMean(rawData);

            // find variance with 1 group
            double total = 0.0;
            for (int i = 0; i < rawData.Length; i++)
            {
                total += Math.Pow((rawData[i][0] - grandMean), 2);
            }
            groupVariation[0] = total / (rawData.Length - 1);

            for (int i = 2; i <= MAX_CLUSTER_SEARCH; i++)
            {
                int[] clustering = Cluster(rawData, i);
                double ssb = 0.0;

                for (int j = 1; j <= i; j++)

                // search for group variance at 1...j clusters
                //  for (int j = 1; j <= MAX_CLUSTER_SEARCH; j++)
                {
                    int numObjects = GetGroupSize(rawData, clustering, j);
                    double groupMean = GetGroupMean(rawData, clustering, j);


                    ssb += numObjects * Math.Pow((groupMean - grandMean), 2);
                    //    ssb += (number of objects in group j) * ((group j mean) - (grand mean))^2

                    // (number of objects in group j)  =  clustering[j].Length ?
                }
                groupVariation[i - 1] = ssb / (i - 1);
            }
            //  variation = SSB / (K - 1)
            //   double variation = ssb / ()
            return groupVariation;
        }
        static double GetGroupMean(double[][] data, int[] clustering, int group)
        {
            double total = 0;
            int count = 0;
            ...
            /*
                        ...
                        for (int k = 0; k < MAX_CLUSTER_SEARCH; ++k)
                        {
                            for (int i = 0; i < data.Length; ++i)
                            {
                                int clusterID = clustering[i];
                                if (clusterID != group) continue;
        ...
        static double GetGrandMean(double[][] data)
        {
            double total = 0;
            int count = 0;
            for (int i = 0; i < data.Length; ++i)
        ...
        static int GetGroupSize(double[][] data, int[] clustering, int group)
        {
            int numInGroup = 0;

            for (int i = 0; i < clustering.Length; i++)
            {
                if (clustering[i] == (group - 1)) numInGroup++;
            }
            /*
            for (int k = 0; k < MAX_CLUSTER_SEARCH; ++k)
            {
